...
const Main = () => {
    ...
    const getAllFeed = (page, more) => {
        ...
        axios.get(`${feed_list}?page_no=${page ? page : 1}&limit=50${sortTabOn === 2 ? '&sort=favorite' : ''}${likeCheck ? '&favorite=1' : ''}${searchValue.length > 0 ? '&search='+searchValue : ''}`,{
            headers: headers,
        })
        .then((res)=>{
            if(res.status === 200){
    ...
    return(<>
        ...
        <div className="square_list_wrap gray_wrap" style={{'padding-top':'100px'}}>
            <div className="cont4">
                <ListTopTitleBox
                    tit='피드 스퀘어'
                />
